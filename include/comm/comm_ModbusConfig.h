...
static void ResetCommumication(TMbPort *hPort, Bool ClearEventLog)
{
	...
	Tout1_5 = ( ( (165 * Scale) / Params->BaudRate) * Params->MpyTout ) + 1; // 165 = 1.5 фрейма * 11 бит в фрейме * 10 масштаб = 165; единицу прибавляем для округления вверх
	Tout3_5 = ( ( (385 * Scale) / Params->BaudRate) * Params->MpyTout ) + 1; // 385 = 3.5 фрейма * 11 бит в фрейме * 10 масштаб = 385; единицу прибавляем для округления вверх
	/*
	...
	*/
	SetTimeout(&Frame->TimerPre,  Tout3_5);
	SetTimeout(&Frame->TimerPost, Tout3_5);
	SetTimeout(&Frame->TimerConn, Params->ConnTimeout * Scale);
	#if defined(_SLAVE_)
	SetTimeout(&Frame->TimerAck,  Params->AckTimeout * Scale);
